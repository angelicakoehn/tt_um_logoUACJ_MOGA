...
async def test_project(dut):
    ...
    async def capture_frame(frame_num, check_sync=True):
        ...
        if check_sync:
            for j in range(V_FRONT):
                dut._log.info(f"Frame {frame_num}, front porch line {j}")
                await check_line(1)
            for j in range(V_SYNC):
                dut._log.info(f"Frame {frame_num}, sync pulse line {j}")
                await check_line(0)
            for j in range(V_BACK):
                dut._log.info(f"Frame {frame_num}, back porch line {j}")
                await check_line(1)
        else:
    ...
    for i in range(CAPTURE_FRAMES):
        ...
        if PIL_AVAILABLE:
            ...
            dut._log.info(f"PIL not available, skipping save of frame {i}")


@cocotb.test()
async def compare_reference(dut):
    if not PIL_AVAILABLE:
        dut._log.warning("PIL not available, skipping image comparison test")
        return
    ...
    for img in glob.glob("output/frame*.png"):
        ...
        if not os.path.exists(ref_path):
            dut._log.warning(f"Reference image {ref_path} not found, skipping")
            continue
